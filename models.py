...
# Adapted from: https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=9912507
class STPNetWithSpikes(nn.Module):
    def __init__(self,
                 input_dim=64,
                 hidden_dim=16,
                 noise_std=0.0,
                 ...
                 syn_tau_y=1.8,
                 syn_betta=0.4375,
                 syn_u0=0.3):     # syn_u: calcium concentration

        super(STPNetWithSpikes, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.syn_tau = syn_tau
        self.syn_u = syn_u
        self.syn_u0 = syn_u0
        self.syn_tau_y = syn_tau_y
        self.syn_betta = syn_betta
    ...
    def init_syn_y(self, batch_size=128):
        return torch.ones([batch_size, self.input_dim])
    ...
    def forward(self, inputs):
        ...
        for i in range(inputs.shape[1]-1):
            ...
            # forward Euler
            heaviside = 1 / (1 + torch.exp(-20 * (self.syn_x - 0.5)))
            u_probability = self.syn_u0 + (self.syn_u - self.syn_u0) / (1 + torch.exp(-50 * (self.syn_y - 0.673)))

            self.syn_y = self.syn_y + -(self.syn_y) / self.syn_tau_y + \
                self.syn_betta * heaviside
            self.syn_x = self.syn_x + (1 - self.syn_x) / self.syn_tau - \
                u_probability * self.syn_x * inputs[:, i]
            # clamp between [0,1]
            self.syn_x = torch.clamp(self.syn_x, min=0, max=1)

            # print(torch.mean(self.syn_x))

            # syn_y_list.append(self.syn_y)
            syn_x_list.append(self.syn_x)

        # print(torch.mean(torch.stack(syn_y_list)))
